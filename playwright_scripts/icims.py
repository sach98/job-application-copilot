...
class IcimsBot(FormFillBot):
    def run(self) -> bool:
        self.log("iCIMS direct filling is currently under construction.")
        self.log(f"candidate: Please apply manually to this position at {self.jd_url}")
        self.screenshot("icims_manual_fallback")
        self.wait_for_approval(stage="manual_fallback")
        return False

def main():
    parser = argparse.ArgumentParser(description="iCIMS form fill bot stub")
    parser.add_argument("--url", required=True, help="iCIMS application URL")
