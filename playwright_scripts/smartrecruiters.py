...
class SmartRecruitersBot(FormFillBot):
    def run(self) -> bool:
        self.log("SmartRecruiters direct filling is currently under construction.")
        self.log(f"candidate: Please apply manually to this position at {self.jd_url}")
        self.screenshot("smartrecruiters_manual_fallback")
        self.wait_for_approval(stage="manual_fallback")
        return False

def main():
    parser = argparse.ArgumentParser(description="SmartRecruiters form fill bot stub")
    parser.add_argument("--url", required=True, help="SmartRecruiters application URL")
